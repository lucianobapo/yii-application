...
$('.search-form form').submit(function(){
	$('#erpnet-cliente-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
...
<h3><?php echo utf8_encode(Yii::t('erpnetUi', 'viewClientesTitle', array(), 'i18n')); ?></h3>
	...
	'columns'=>array(
		...
		*/
		array(
			...
			'buttons'=>array(
				'view'=>array(
					'url'=>'Yii::app()->createUrl(Controller::getModuleName()."/erpnetCliente/view", array("id"=>$data->id,"asDialog"=>1))',
					'options'=>array(
						'ajax'=>array(
							'type'=>'POST',
				...
				'delete'=>array(
					'visible'=>'ErpnetCliente::model()->findByPk($data->id)->ativado',
					'label' => 'Desativar',
				),
				'ativar'=>array(
					'visible'=>'(!ErpnetCliente::model()->findByPk($data->id)->ativado)',
					'label' => 'Ativar', // text label of the button
					'url' => 'Yii::app()->createUrl(Helpers::getModuleName()."/erpnetCliente/ativar", array("id"=>$data->id))', //Your URL According to your wish
			...
			'updateDialog'=>array(
				'controllerRoute' => Controller::getModuleName().'/erpnetCliente/update', //=default
